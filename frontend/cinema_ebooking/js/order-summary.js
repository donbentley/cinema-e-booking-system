...
if (orderData && orderData.seats.length > 0) {
    document.getElementById('selectedMovieTitle').textContent = `Movie: ${orderData.movie}`;  // Display movie title
    ...
    orderData.seats.forEach(ticket => {
        ...
        ticketDetailDiv.innerHTML = `
            ...
            <select id="age-${ticket.seat}" name="age-${ticket.seat}" class="ticket-type" onchange="updateTicketPrice('${ticket.seat}')" required>
                <option value="Adult" ${ticket.age === 'Adult' ? 'selected' : ''} data-price="12.00">Adult - $12.00</option>
                <option value="Child" ${ticket.age === 'Child' ? 'selected' : ''} data-price="8.00">Child - $8.00</option>
                <option value="Senior" ${ticket.age === 'Senior' ? 'selected' : ''} data-price="8.00">Senior - $8.00</option>
            </select>
            | Price: <span id="price-${ticket.seat}">${ticket.price}</span>
            <button onclick="editTicket('${ticket.seat}')">Edit</button>  <!-- Edit button first -->
            <button onclick="removeTicket('${ticket.seat}')">Remove</button>  <!-- Remove button second -->
        `;
...
function editTicket(seat) {
    ...
    if (ticketToEdit) {
        ...
        const queryString = `book-movie.html?movie=${encodeURIComponent(selectedMovie)}&showtime=${encodeURIComponent(showtime)}&showdate=${encodeURIComponent(showdate)}&seat=${seat}&age=${ticketToEdit.age}&step=seats`;
...
function updateTicketPrice(seat) {
    ...
    if (ticketType && priceElement) {
        ...
        const ticketIndex = orderData.seats.findIndex(ticket => ticket.seat === seat);
        if (ticketIndex !== -1) {
