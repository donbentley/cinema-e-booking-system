...
window.onload = function () {
	...
	if (selectedMovie) {
		document.getElementById(
			"selectedMovieTitle"
		).textContent = `Booking for: ${selectedMovie}`;
	} else {
	...
	if (showdate) {
		...
		document.getElementById(
			"displaySelectedDate"
		).textContent = `Selected Date: ${selectedDateText}`;
	}
	...
	if (showtime) {
		...
		document.getElementById(
			"displayDateTimeSeats"
		).textContent = `Selected Date: ${selectedDateText}, Time: ${selectedTimeText}`;
	}
...
document.getElementById("showtime").addEventListener("change", function () {
	if (this.value) {
		...
		const displayDateTimeSeats = document.getElementById(
			"displayDateTimeSeats"
		);
...
function updateTicketPrice(seat) {
	...
	if (ticketType && priceElement) {
		...
		console.log(
			`Seat: ${seat}, Ticket Type: ${selectedOption.value}, Price: $${price}`
		);
